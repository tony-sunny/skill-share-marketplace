...
describe('auth controller', () => {
  ...
  describe('signUp', () => {
    it('should create user if not exists', async () => {
      ...
      const req: any = { body: { email: 'a@b.com', password: 'pw', foo: 'bar' } };
      const res = mockRes();
      await signUp(req, res);
      expect(findUserByEmail).toHaveBeenCalledWith('a@b.com');
      expect(bcrypt.hash).toHaveBeenCalledWith('pw', 10);
      expect(createUser).toHaveBeenCalledWith({ foo: 'bar', email: 'a@b.com', password_hash: 'hashed' });
  ...
  describe('login', () => {
    it('should return token if credentials valid', async () => {
      (findUserByEmail as jest.Mock).mockResolvedValue({ id: 1, email: 'a@b.com', role: 'user', password_hash: 'hashed' });
